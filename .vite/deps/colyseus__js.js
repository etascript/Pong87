...
Schema instances may only have up to 64 fields.`);
      ...
      const _CollectionSchema = class _CollectionSchema {
         ...
         * - Then, the encoder iterates over all "owned" properties per instance and encodes them.
         */
        static [(_a$2 = $encoder, _b$2 = $decoder, $filter)](ref, index, view2) {
          return !view2 || typeof ref[$childType] === "string" || view2.isChangeTreeVisible((ref[$getByIndex](index) ?? ref.deletedItems[index])[$changes]);
        }
        static is(type2) {
      ...
      _CollectionSchema[_a$2] = encodeKeyValueOperation;
      _CollectionSchema[_b$2] = decodeKeyValueOperation;
      ...
      const _SetSchema = class _SetSchema {
        ...
        add(value) {
          ...
          const operation = this[$changes].indexes[index]?.op ?? exports2.OPERATION.ADD;
          this[$changes].indexes[index] = index;
          this.$indexes.set(index, index);
          this.$items.set(index, value);
      ...
      _SetSchema[_a$1] = encodeKeyValueOperation;
      _SetSchema[_b$1] = decodeKeyValueOperation;
...
Check @type() annotation`);
      ...
      const _Schema = class _Schema {
         ...
         */
        static [(_a = $encoder, _b = $decoder, $track)](changeTree, index, operation = exports2.OPERATION.ADD) {
          changeTree.change(index, operation);
        }
        /**
         * Determine if a property must be filtered.
        ...
        static debugRefIds(ref, showContents = false, level = 0, decoder2, keyPrefix = "") {
          ...
          const refCount = root?.refCount?.[refId] > 1 ? ` [×${root.refCount[refId]}]` : "";
...
`;
      ...
      _Schema[_a] = encodeSchemaOperation;
      _Schema[_b] = decodeSchemaOperation;
      let Schema = _Schema;
        ...
        encode(it = { offset: 0 }, view2, buffer = this.sharedBuffer, changeSetName = "changes", isEncodeAll = changeSetName === "allChanges", initialOffset = it.offset) {
          ...
          while (current = current.next) {
            ...
            for (let j = 0; j < numChanges; j++) {
              ...
              const operation = isEncodeAll ? exports2.OPERATION.ADD : changeTree.indexedOperations[fieldIndex];
        ...
        decoder2.triggerChanges = function(allChanges) {
          ...
          for (let i = 0, l = allChanges.length; i < l; i++) {
            ...
            if ((change.op & exports2.OPERATION.DELETE) === exports2.OPERATION.DELETE && change.previousValue instanceof Schema) {
              const deleteCallbacks = callbacks[$root.refIds.get(change.previousValue)]?.[exports2.OPERATION.DELETE];
      ...
      class StateView {
        ...
        add(obj, tag = DEFAULT_VIEW_TAG, checkIncludeParent = true) {
          ...
          if (!changeTree) {
            ...
            return false;
          } else if (!parentChangeTree && changeTree.refId !== 0) {
          ...
          if (tag !== DEFAULT_VIEW_TAG) {
            ...
            for (let i = 0, len = changeSet.operations.length; i < len; i++) {
              ...
              if (op !== exports2.OPERATION.DELETE && (isInvisible || // if "invisible", include all
        ...
        remove(obj, tag = DEFAULT_VIEW_TAG, _isClear = false) {
          ...
          if (tag === DEFAULT_VIEW_TAG) {
            ...
            if (parent && !Metadata.isValidInstance(parent) && changeTree.isFiltered) {
              ...
              } else if (changes2[changeTree.parentIndex] === exports2.OPERATION.ADD) {
              ...
              metadata?.[$viewFieldIndexes]?.forEach((index) => changes[index] = exports2.OPERATION.DELETE);
            }
          } else {
            metadata?.[$fieldIndexesByViewTag][tag].forEach((index) => changes[index] = exports2.OPERATION.DELETE);
          }
